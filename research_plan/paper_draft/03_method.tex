...
of \emph{which} objectives help under missingness (Section~\ref{sec:experiments})
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
giving the masked feature matrix $\bar{\mathbf{X}}$. Following the missing-completely-
at-random (MCAR) mechanism, $V_m$ is sampled uniformly at a rate $\rho=|V_m|/n$.
...
Feature Propagation~[FP]: features are diffused over the graph while observed
...
The encoder $f_\theta$ is an $L$-layer GraphSAGE~[GraphSAGE] network. At layer
